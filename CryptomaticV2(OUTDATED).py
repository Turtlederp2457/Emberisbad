...
def bruteForceOption():
    ...
    for line in encryptedTxtFile:
        for i in line:
            biEncryptedTxtList.append(charToBinary(i))
    #Looping through each key
    for key in possibleKeys:
        keyIndex = 0
        biPlainTxtList = []
        #unencrypts for a key
        for i in biEncryptedTxtList:
            biPlainTxtList.append(charXOR(i, key[keyIndex]))
            if keyIndex==0: keyIndex=1
            else: keyIndex=0
        #converts to text 
        plainTxtList = []
        for i in biPlainTxtList:
            plainTxtList.append(binaryToAscii(i))
        #checks to see if its a match
        ...
        bigCount = 0
        for word in words:
            count = possibleString.count(word)
            bigCount += count
        if 
